...
class ImagePathToTensor(object):
    ...
    def __call__(self, path):
        ...
        #[k1,k2,k3,k4] = [ran.randint(0, 2**32 -1) for _ in range(4)]

        cap2 = add_gaussian_noise(cap,self.tile) #resize_video_clip2(color_shuffle(apply_color_inversion(rot_and_inv(shuffle_blocks(resize_video_clip1(cap),k1),k2),k3),k4))
                
        cap = cap2.copy()

        cap = torch.from_numpy(cap)
        cap = cap.permute(-1, 0, 1, 2)
        cap = cap.float() 
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        im = self.dataframe.iloc[index].path
